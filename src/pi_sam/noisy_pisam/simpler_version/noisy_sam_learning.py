...
from sam_learning.core.matching_utils import extract_discrete_effects_partial_observability, \
    extract_not_effects_partial_observability, extract_discrete_effects, extract_not_effects
...
class NoisySAMLearner(SAMLearner):
    """
    A SAM learner with:
    ...
    If *no* patches are given, conflicts may still arise from SAM itself
    ...
    def _should_negate_grounded_effect(
        ...
        This corresponds exactly to the SAM semantics:
    ...
    def handle_effects(self, grounded_action: ActionCall, previous_state: State, next_state: State) -> None:
        """
        SAM effect handling with conflict detection for:
        ...
        SKIP the regular SAM effect update for this transition.
        """
        action_name = grounded_action.name
        ...
        cannot_be_effects: Set[GroundedPredicate] = extract_not_effects(prev_preds)
        ...
        self._handle_effects(grounded_action, previous_state, next_state)

    def _handle_effects(self, grounded_action: ActionCall, previous_state: State, next_state: State) -> None:
        """
        ...
        cannot_be_effects = extract_not_effects(
            get_state_grounded_predicates(previous_state)
        )
