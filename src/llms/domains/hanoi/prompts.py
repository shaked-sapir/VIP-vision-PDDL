...
Using ONLY PEG_STACKS from STAGE 1, output:

VALID PREDICATES:

- on_disc(x:disc,y:disc)  → disc x is directly on top of disc y
- on_peg(x:disc,y:peg)    → disc x is directly on peg y (the lowest disc in that column)
- clear_disc(x:disc)      → x is the top disc in its column
- clear_peg(x:peg)        → no discs in that column
