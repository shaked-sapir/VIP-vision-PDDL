...
- Algorithms: `benchmark/algorithms.py` — `cdps` (our learner + conflict search) plus baseline keys from `benchmark/baselines/BASELINE_REGISTRY` (`rosame`, `rosame_i`, `rosame_milp`, `rosame_milp_tag`, `rosame_milp_base`). Legacy run configs may use `baselines: [rosame]` (implies CDPS too). ROSAME+MILP encoding rule-sets are bundled in `algorithm_adapters/rosame_milp/encoding_config.py` (`MilpEncodingConfig`, presets `upstream()`/`tag()`); a new MILP variant = a new preset + a thin runner subclass + one registry entry.
