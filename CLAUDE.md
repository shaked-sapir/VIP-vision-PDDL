...
| `src/trajectory_handlers/` | Image→trajectory pipeline per domain. Split into inference base, PDDLGym source, external source, LLM mixin layers, `frame_classification.py` (`FrameClassifier` protocol: sequential or concurrent per-frame VLM calls, results always in frame order), and `pddlgym_problem_generator.py` (ROSAME-style problem generation). |
...
| `src/milp/` | The CP-SAT encoder and its vocabulary, shared by two unrelated learners (`pisam_milp_*` and the `rosame_milp_24*` / `rosame_i_milp_24` / `rosame_i_milp_26` baselines) — hence `src/`, not under either one. `encoder.py` (`CPSATObservedActions`), `encoding_config.py` (`MilpEncodingConfig`, presets `upstream()`/`tag()`/`cdps_dialect()`), `converter.py` (pddl_plus → `planning_structs`, `GtAnchoring`, `RepeatedArgsInstance`, `problem_object_types` — the declared-type overlay that stops trajectory-inferred object types from making an observed action ungroundable; `observation_to_trace` **raises**, `try_observation_to_trace` returns `None` and is only for the loop's optional warm-start hints; `cv_predictions_to_trace` builds a vendored trace from ROSAME-I CV logits), `domain_assets.py` (regenerates Convertor/DL vocabularies from `src/domains/*.pddl`; `python -m src.milp.domain_assets`), `head_alignment.py` (CP proposition/action index ↔ ICAPS-26 DL head index — the two ground the same domain in different argument orders, so the ICAPS-26 arms must map through `rosame_argument_permutation`; aligning by predicate name alone breaks on depot, gripper, hanoi and npuzzle. The ICAPS-24 arms are unaffected; `proposition_index_by_name` / `action_index_by_name` are the string-keyed form the ICAPS-26 adapter consumes), `schema_row_alignment.py` (ICAPS-26 schema-head `forward()` row ↔ CP `predicate_arguments` row; the two enumerate the same bindings in different orders, so a positional zip silently mislabels — `schema_row_keys` is the bijection `Rosame26MipRepairer` consumes), `trace_tensors.py` (ragged image traces → the vendored ICAPS-26 network's `img_traces` / `state_traces` / `action_traces` plus a per-trace `lengths` array. Owns the §4.1 frame arithmetic — `interior_frame_count`, both endpoints dropped so `T = N-1`, a 2-image trace rejected — which `test_vendor_net_contract.py`'s gate 1 imports rather than restates. Interior `state_traces` rows are zero filler, so the network's `state_acc` is not an accuracy), `rosame26_model.py` (`Rosame26Goal` = `ObservedActionMixin` + `LengthMaskedLossMixin` over the vendored `ROSAMEGoal`. The first replaces the softmax `a` with the observed one-hot without touching `forward`; the second reads a per-trace `lengths` so a right-padded batch is scored on its true steps — `live_action_steps` (`t <= L`), `live_prefix_steps` (`t < L`), `total_action_steps` as the normaliser. `lengths=None` is the vendored loss exactly, and `test_vendor_net_contract.py`'s gate 2a pins that. Dropping a mixin is how you get unsupervised actions back), `loss_convergence.py` (the training-loss plateau rule every ROSAME arm stops on: `window_best` / `relative_improvements` / `has_converged`, the `LossConvergenceRule` dataclass a `rosame_convergence:` config block parses into, and the stop-reason constants; reads training loss only, never test data), `rosame26_budget.py` (the plan §1.2 pre-flight — projects a schedule's cost from the *measured* solve and DL-epoch costs rather than `mip_time_limit`, multiplied by `n_seeds`, and refuses to start when it does not fit the cell timeout, naming the epoch count that would. The configured epoch count is a ceiling this lowers, not a derived constant), `rosame26_emitter.py` (a trained `Domain_Model` → PDDL in the *reference domain's* argument order — `extract_pddl` writes the sorted signature and AMLGym scores positionally, so four of five domains would score ~0; also fixes two defects that make the vendored output unparseable, bare variables in the bodies and `()` for an empty block. `check_not_degenerate` is gate 4, the empty-effects guard), `rosame26_repairer.py` (`Rosame26MipRepairer` — the injected `MipRepairer` over `CPSATObservedActions`, not the vendored Convertor, whose solvers take `max_t` from the first trace of a ragged fold), `rosame26_training.py` (`Rosame26Trainer` — the harness, overriding `train`/`_run_training` only. Owns the pinned run parameters, cuda-else-cpu device selection, seeded batching, and the MILP-as-injected-`MipRepairer` protocol; `pre_mip_epoch >= epoch` is the DL-only arm. No augmentation transforms, no `evaluate` — it would reach `model_permutation`), `vendor/` (upstream ROSAME+MILP: `planning_structs`, `constraint_opt`, `dl/`, `convertor/` — see `vendor/UPSTREAM.md`). Importing `src.milp` is what puts `vendor/` on `sys.path`, so it is the precondition for any bare `from planning_structs...` import. |
...
| `benchmark/experiment_running_helpers/` | Fold execution glue: `run_fold.py` (spine), `data_source.py` (image vs simulated), `learning_helpers.py` (CDPS + both `pisam_milp_*` denoisers), `trajectory_utils.py` (fold prep / anchored GT), `normalize.py` (hyphen→underscore identifier rewrite for image corpora), `resume.py` (fold I/O + `run_params` checks), `gt_builder.py` (GT export/validation), `collect_results.py` + `result_schema.py` (per-cell JSON → tables), `planning_copy.py` (`solving_metrics` — the one place a learned model reaches the planner. It plans on a copy without no-effect operators: unified-planning writes such an action with no `:effect` field and Fast Downward rejects the whole domain with exit code 31, which AMLGym's `problem_solving` counts in no bucket, so the row read 0 for every outcome. Removing a no-effect operator changes neither solvability nor plan validity. Syntactic and predictive metrics keep reading the untouched model. Also emits `planning_syntax_error_ratio`, `planning_error_ratio`, `planning_dropped_operators`; `evaluation.evaluate_model` and `evaluation/multi_solution_evaluator.py` both go through it). |
| `benchmark/algorithm_adapters/` | ROSAME family glue used by `baselines/`: `po_rosame_runner.py` (symbolic PO ROSAME), `rosame_i_runner.py` (imaged ROSAME-I), `rosame_milp/` (symbolic MILP loop + `milp_loop_i.py` for ROSAME-I+MILP), `anytime_snapshots.py` (`SnapshotWriter` — intermediate ROSAME models for the anytime curves), `best_checkpoint.py` (`BestModelTracker` — the lowest-training-loss snapshot of a network's `action_schemas`, shared by the ICAPS-24 and ICAPS-26 arms; `reset()` restarts it at a MILP arm's first successful solve). Every pooled ROSAME loop takes `stop_check` / `timeout_check` / `tracker` hooks and reports `stop_reason` in one vocabulary (`converged` | `epochs_exhausted` | `timeout` | `agreement_reached` | `no_usable_traces`); `po_rosame_runner.learn_pooled` returns a `TrainingReport`, and `learn_full_with_report` returns the PDDL with it (`learn_full` still returns the string). The MILP loops score only the loss series after their first successful solve, where the pseudo-label term joins the loss. The encoder itself lives in `src/milp/`. CDPS calls `ConflictDrivenPatchSearchPISAM` directly from `learning_helpers.py`. `lamanna/` is the glue shared by the OffLAM and NOLAM arms (both pip packages by Lamanna et al., parsing one trace grammar): `traces.py` (`write_partial_trace` — a masked `Observation` → explicit positive/negative literals, masked atoms omitted, one state per line), `subprocess_learn.py` (`run_learner` — the learner call in a child process with its own cwd and a wall-clock kill; neither package takes a budget, both `exit()` on a parse error and write `tmp/`/`PDDL/` relative to the cwd), `realised_rates.py` (`realised_rates` — the fold's hidden and flipped atom fractions measured against `gt_trajectories/`; the `percentage` strategies round, so the nominal `p` is not what the data carries), `domain_dialect.py` (`write_typed_domain` — the copy the learners read: untyped parameters typed `- object`, the flat type list rooted at `object`; their regex parsers take arity from typed parameters, so depot's `(clear ?x)` otherwise becomes `(clear)`), `nolam_glue.py` / `offlam_glue.py` (the in-child functions; the NOLAM one drops operators left with no preconditions and no effects, since an empty `(and)` makes Fast Downward fail with an error the solving metric cannot classify), `probes.py` (test-only targets for the harness). Never go through `amlgym.algorithms.*` adapters: their preprocessing turns a masked atom into an explicit false. |
| `benchmark/baselines/` | Pluggable competitor runners (`BaselineRunner` ABC) registered in `BASELINE_REGISTRY` (`rosame_24`, `rosame_i_24`, `rosame_i_26`, `rosame_i_milp_24`, `rosame_i_milp_26`, `rosame_milp_24`, `rosame_milp_24_tag`, `nolam`, `offlam`). `regime.py` — `DegradationRegime` (a cell's source + nominal mask/noise rates + corpus root) and `gate_baselines`: each runner's `supports(regime)` decides whether it runs in a cell, `for_regime(regime)` binds it; `benchmark_runner` gates per cell, `experiment_runner` records the drops in `run_params.json` (`skipped_baselines`), `backfill_baseline` gates from each cell's recorded `p_mask`/`p_noise`; `baseline_regime_gate: off` disables it. `lamanna_runner_base.py` (`LamannaBaselineRunner` — workspace, trace writing, realised rates, child-process call) ← `nolam_runner.py` (`NOLAMRunner`, mask=0 cells only; `nolam_noise: oracle` hands it the realised flip rate, a float pins `e` and suffixes the row `NOLAM__e=<v>`; `nolam_allow_neg_precs` selects MAP vs MAP_pre+) and `offlam_runner.py` (`OffLAMRunner`, noise=0 cells only, no knobs, the paper's setting 1 = partial states, full actions). Plan and rationale: `docs/offlam-nolam-baselines-plan.md`. `rosame26_runner.py` is the ICAPS-26 imaged arm, DL-only: the fold adapter below, `Rosame26Trainer` with no `mip_repairer` injected, and `src/milp/rosame26_emitter.py` for emission, guarded by its `check_not_degenerate`. `rosame26_milp_runner.py` (`Rosame26MilpRunner`) is the same 26 arm with the solver on — it overrides `pre_mip_epoch` / `make_repairer` to inject `Rosame26MipRepairer`. `image_fold_inputs.py` is the fold walk every imaged arm shares — `resolve_fold_inputs` turns a fold's prepared trajectories into `ResolvedTrace`s (problem, frames, observed actions, both GT endpoint states), plus the dialect-conforming parsers and `infer_bench_key`. The GT anchors come from `gt_trajectories/`, never from the degraded trajectory's own states. `rosame26_data.py` is the ICAPS-26 fold adapter sitting on top of it — corpus object union → one shared `Grounding` (via `write_grounding_assets` into a scoped root), corpus-level image standardisation cached on `(domain, resize)` beside the data, and `build_fold_batch` → a `FoldBatch` wrapping `src/milp/trace_tensors.py`'s `PaddedTraces` plus the kept/dropped split. Every proposition and action key crosses through its `canonical()` (`src/utils/pddl_naming.py`) on both sides. |
...
- `__init__(domain_name, frame_classifier=None)` — default `SequentialFrameClassifier`; pass `ConcurrentFrameClassifier` to classify a window's frames together. Either way the result is one predicate dict per frame, in frame order.
...
### Frame Classifier
```python
# src/trajectory_handlers/frame_classification.py
class FrameClassifier(Protocol):
    def classify_frames(self, classifier, image_paths: List[Path]) -> List[Dict]: ...
```
- Returns one predicate dict per frame **in frame order**. Completion order must not leak into the trajectory.
- Concrete strategies: `SequentialFrameClassifier` (default) and `ConcurrentFrameClassifier` (all-or-nothing per window; one exhausted frame fails the whole window).
- Do not put this in `FluentClassifier` — that class classifies a single image.
...
| `utils/pddl_trajectory.py` | `parse_trajectory_with_declared_types` (parse a `.trajectory` with the problem's declared object types; without a problem the parser infers types from the init state and can mistype an object), `build_trajectory_file`, `export_gt_trajectory`, `observation_to_trajectory_file`, `ensure_trajectory_json`, `extract_actions_from_trajectory_json`, `propagate_frame_axioms_in_trajectory`, `propagate_frame_axioms_in_memory`, `propagate_frame_axioms_selective`, `inject_gt_states_by_percentage` |
...
| `utils/pddl_naming.py` | `canonical`, `spelling_map`, `rewrite_symbols` — hyphen vs underscore PDDL identifier dialects |
| `utils/prompt_cache.py` | `prompt_cache_key` — stable LLM prompt-prefix routing key (`"<task>_<12 hex>"`) |
...
- **Strategy** — masking via `MaskingStrategy`; noising via `NoisingStrategy`; per-window frame classification via `FrameClassifier`; trace generation via `TraceSource`. Algorithm selection via `benchmark/algorithms.py` (`cdps` / `cdps_anchored` / `pisam_milp_single_round` / `pisam_milp_loop` + baseline keys from `benchmark/baselines/`).
...
- **Domain assets** — ROSAME Convertor/DL vocabularies are generated from `src/domains/*.pddl` via `src/milp/domain_assets.py`, never maintained by hand in `vendor/`. Static specs (`planning_structs/specs/`, `vendor/pddl/`) are checked in; per-fold `domain_model.json` / `objects.json` are written at run time.
...
- Entry points:
  ...
  - `benchmark/experiment_runner.py` — single experiment (one domain, one data source); `--algorithms cdps rosame_24` (default)
  ...
  - ROSAME convergence is config too, and off unless asked for: `shared.rosame_convergence` (`window` / `min_improvement` / `patience` / `min_epochs` → `LossConvergenceRule`), `shared.epochs` (a ceiling once the rule is on), `shared.agreement_stop` (blank = agreement is recorded, never a stop rule) and `shared.train_per_trajectory: false` (the per-trajectory schedule has no single loss curve, so the rule does not apply to it). `run_config_large.yaml` sets all four; runner defaults are the legacy behaviour (rule off, `agreement_stop` 1.0, 100 epochs), so small-data rows stay reproducible and `run_params()` gains keys only when the rule is on. With the rule on an arm emits its best-loss checkpoint, enforces the fold's `timeout_seconds`, and writes its per-epoch loss to `<fold>/rosame_training/<arm>.json`, which `evaluation/cfm/convergence.py` reads in preference to snapshots — so a large sweep needs no `snapshot_interval`. Mirrored by `backfill_baseline.py --rosame-convergence '<json>' / --agreement-stop / --epochs / --no-train-per-trajectory`. Plan and status: `docs/rosame-training-convergence-fix.md`.
...
- `benchmark/backfill_cdps.py` — retrofit `cdps_anchored` / `pisam_milp_single_round` / `pisam_milp_loop` into existing cells via the same frozen-observation path (`backfill_common.py`); MILP knobs from `--milp-config`
  - `benchmark/repair_solving_metrics.py` — re-score only the solving fields of records whose four planning outcomes are all zero (written before `planning_copy.py`), from the models on disk: `fold_result.json` rows and every `all_solutions_metrics.json` entry. `--dry-run` counts; never run it while a backfill writes the same cells
...
- Algorithms: `benchmark/algorithms.py` — `cdps` (init GT), `cdps_anchored` (init+final GT via data prep), `pisam_milp_single_round` (one CP-SAT solve + PI-SAM), `pisam_milp_loop` (repeated subset solves, keep best V), plus baseline keys from `benchmark/baselines/BASELINE_REGISTRY` (`rosame_24`, `rosame_i_24`, `rosame_i_26`, `rosame_i_milp_24`, `rosame_i_milp_26`, `rosame_milp_24`, `rosame_milp_24_tag`). `resolve_algorithms` → `(run_cdps, run_cdps_anchored, run_pisam_milp, run_pisam_milp_loop, baseline_runners)`. Legacy run configs may use `baselines: [rosame_24]` (implies CDPS too). MILP encoding rule-sets are bundled in `src/milp/encoding_config.py` (`MilpEncodingConfig`, presets `upstream()`/`tag()`/`cdps_dialect()`); a new MILP variant = a new preset + a thin runner/driver + one registry entry.
- MILP denoisers (`pisam_milp_single_round` / `pisam_milp_loop`): configured by the `pisam_milp:` block of `run_config.yaml` → `PisamMilpConfig` (validated even when the arm is off). The selected algorithm key pins `variant`, so one block can serve both arms. Single-round: `learning_helpers.learn_pisam_milp_single_round()` → `milp_denoiser/single_round.run_single_round()`. Loop: `learn_pisam_milp_loop()` → `milp_denoiser/loop.run_loop()` (sampled subsets, GT-free V selection, `StopRules`). Both dispatched by `run_fold.run_cdps_phase(...)` so they share CDPS's evaluation path. Row labels are arm-suffixed (`pisam_milp_algorithm_name`); artifacts land under `<fold>/pisam_milp_single_round/` or `<fold>/pisam_milp_loop/` in CDPS's shape plus `milp_repair_log.json` / `milp_masked_completion.json` (the loop also writes `milp_loop_rounds.json` / `milp_loop_round_models/`). `repair_cost` was documented as a lower bound on any CDPS conflict-free model's cost — **that check is vacuous and has been retired** (design §7.1a): CDPS may also add model constraints, which the MILP cannot, so its models are not MILP-feasible and the bound does not follow. When comparing repair magnitudes across arms read `net_cost` / `net_fluent_patch_count` (`plan_denoising/patch_accounting.py`), never `cost` — the latter double-charges self-cancelling patch pairs.
- Learning path: `learning_helpers.learn_cdps()` → `CDPSConfig` + `ConflictDrivenPatchSearchPISAM.run(..., gt_states_by_obs=..., timeout_seconds=...)` on masked observations (no `NOISY_PISAM` adapter; SAM-only paths removed). MILP arms: `learn_pisam_milp_single_round()` / `learn_pisam_milp_loop()`. Optional `--events-tracing` / `events_tracing` writes `search_trace.json` per fold via `benchmark/diagnosis/trace_serialization.py`.
...
- Evaluation: `benchmark/evaluation/experiment_report.py` (`fully-detailed-report.xlsx`), `benchmark/evaluation/cfm/cfm_quality_analysis.py`, `cfm_quality_table.py`, `cfm_domain_aggregate.py`, `build_dashboard.py`, `combine_dashboard_reports.py` (reads `dashboard_config.yaml`), `export_dashboard_shared_metrics.py` (cross-domain shared-metrics xlsx), `predictive_metrics.py` + `upenv_compat.py` (predictive-power eval), `fold_filter.py`, `trajectory_fluent_confusion.py`, `compare_original_observations.py`, `correlation_analysis.py`, `milp_arm_audit.py` (MILP arm A/B + the design §7.1 bound; source of the numbers in design doc §7.1a/§7.1b), `anytime/` (post-hoc anytime performance profiles: `checkpoints.py` reader → `score.py` GT-free scorer → `curves.py` → `run_anytime.py` CLI)
...
- Do not hand-edit `src/milp/vendor/planning_structs/specs/` or `vendor/pddl/` — regenerate with `python -m src.milp.domain_assets`.
