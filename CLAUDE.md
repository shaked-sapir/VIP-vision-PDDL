...
| `benchmark/algorithm_adapters/` | ROSAME PO encoding (`po_rosame_runner.py`) and `rosame_milp/` (the ROSAME↔MILP bridge + milp loop; the encoder itself lives in `src/milp/`). Used by `baselines/`. CDPS calls `ConflictDrivenPatchSearch` directly from `learning_helpers.py`. |
