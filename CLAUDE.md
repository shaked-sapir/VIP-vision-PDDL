...
| `benchmark/baselines/` | Pluggable competitor runners (`BaselineRunner` ABC) registered in `BASELINE_REGISTRY` (`rosame_24`, `rosame_i_24`, `rosame_i_milp_24`, `rosame_milp_24`, `rosame_milp_24_tag`). `image_fold_inputs.py` is the fold walk every imaged arm shares — `resolve_fold_inputs` turns a fold's prepared trajectories into `ResolvedTrace`s (problem, frames, observed actions, both GT endpoint states), plus the dialect-conforming parsers and `infer_bench_key`. The GT anchors come from `gt_trajectories/`, never from the degraded trajectory's own states. |
