...
| `benchmark/experiment_running_helpers/` | Fold execution glue: `run_fold.py` (spine), `data_source.py` (image vs simulated), `learning_helpers.py` (CDPS + both `pisam_milp_*` denoisers), `trajectory_utils.py` (fold prep / anchored GT), `resume.py` (fold I/O + `run_params` checks), `gt_builder.py` (GT export/validation), `collect_results.py` + `result_schema.py` (per-cell JSON → tables). |
...
| `benchmark/baselines/` | Pluggable competitor runners (`BaselineRunner` ABC) registered in `BASELINE_REGISTRY` (`rosame_24`, `rosame_i_24`, `rosame_i_26`, `rosame_i_milp_24`, `rosame_milp_24`, `rosame_milp_24_tag`). `rosame26_runner.py` is the ICAPS-26 imaged arm, DL-only: the fold adapter below, `Rosame26Trainer` with no `mip_repairer` injected, and `src/milp/rosame26_emitter.py` for emission, guarded by its `check_not_degenerate`. `image_fold_inputs.py` is the fold walk every imaged arm shares — `resolve_fold_inputs` turns a fold's prepared trajectories into `ResolvedTrace`s (problem, frames, observed actions, both GT endpoint states), plus the dialect-conforming parsers and `infer_bench_key`. The GT anchors come from `gt_trajectories/`, never from the degraded trajectory's own states. `rosame26_data.py` is the ICAPS-26 fold adapter sitting on top of it — corpus object union → one shared `Grounding` (via `write_grounding_assets` into a scoped root), corpus-level image standardisation cached on `(domain, resize)` beside the data, and `build_fold_batch` → a `FoldBatch` wrapping `src/milp/trace_tensors.py`'s `PaddedTraces` plus the kept/dropped split. Every proposition and action key crosses through its `canonical()` on both sides. |
...
- **Strategy** — masking via `MaskingStrategy`; noising via `NoisingStrategy`; trace generation via `TraceSource`. Algorithm selection via `benchmark/algorithms.py` (`cdps` / `cdps_anchored` / `pisam_milp_single_round` / `pisam_milp_loop` + baseline keys from `benchmark/baselines/`).
...
- Algorithms: `benchmark/algorithms.py` — `cdps` (init GT), `cdps_anchored` (init+final GT via data prep), `pisam_milp_single_round` (one CP-SAT solve + PI-SAM), `pisam_milp_loop` (repeated subset solves, keep best V), plus baseline keys from `benchmark/baselines/BASELINE_REGISTRY` (`rosame_24`, `rosame_i_24`, `rosame_i_26`, `rosame_i_milp_24`, `rosame_milp_24`, `rosame_milp_24_tag`). `resolve_algorithms` → `(run_cdps, run_cdps_anchored, run_pisam_milp, run_pisam_milp_loop, baseline_runners)`. Legacy run configs may use `baselines: [rosame_24]` (implies CDPS too). MILP encoding rule-sets are bundled in `src/milp/encoding_config.py` (`MilpEncodingConfig`, presets `upstream()`/`tag()`/`cdps_dialect()`); a new MILP variant = a new preset + a thin runner/driver + one registry entry.
