"""
Quick smoke test for the Depot LLM object detector and fluent classifier.

Usage:
    python tests/test_depot_llm_components.py <images_dir>
...
Examples:
    ...
    python tests/test_depot_llm_components.py benchmark/data/depot/problem0/

    # Object detection only:
    python tests/test_depot_llm_components.py benchmark/data/depot/problem0/ --skip-classification

    # Classification only (uses manual object list):
    python tests/test_depot_llm_components.py benchmark/data/depot/problem0/ --skip-detection

    # Classify a single image instead of all:
    python tests/test_depot_llm_components.py benchmark/data/depot/problem0/ --classify-index 3

    # Use Gemini:
    python tests/test_depot_llm_components.py benchmark/data/depot/problem0/ --vendor google
"""
...
def _find_all_state_images(images_dir: Path) -> list[Path]:
    ...
    # Sort numerically by extracting the index
...
def test_object_detection(images_dir: Path, vendor: str = "openai") -> dict[str, list[str]]:
    """Test the depot object detector on the first image."""
    ...
    detector = LLMDepotObjectDetector(
        llm_backend=backend,
...
def test_fluent_classification(
    ...
    """Test the depot fluent classifier on one or all images.
    ...
    classifier = LLMDepotFluentClassifier(
        llm_backend=backend,
        init_state_image_path=init_image,
...
def main():
    parser = argparse.ArgumentParser(description="Test Depot LLM components")
