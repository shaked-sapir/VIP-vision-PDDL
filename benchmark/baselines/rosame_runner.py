...
class RosameBaselineRunner(BaselineRunner):
    ...
    def learn(
        ...
        try:
            ...
            rosame = Rosame_Runner(str(domain_path))
...
class PORosameBaselineRunner(BaselineRunner):
    ...
    def learn(
        ...
        try:
            partial_domain = DomainParser(domain_path, partial_parsing=True).parse_domain()
            po_rosame = PORosame_Runner(str(domain_path))
            ...
            for traj_idx, traj_path_str in enumerate(traj_paths):
                ...
                po_rosame.ground_new_trajectory()
                _time_step("learn_rosame_single", lambda: po_rosame.learn_rosame(masked_observation))

            model = po_rosame.rosame_to_pddl()
            if model and ":action" in model:
                return model, {}
