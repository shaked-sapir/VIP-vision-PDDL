...
class TestRosameSeries:
    ...
    def test_agreement_is_carried_when_present(self, tmp_path):
        inst = _instance(tmp_path)
        self._write(inst, "ROSAME_MILP_24", [
            {"epoch": 50, "loss": 2.0, "agreement": None},
            {"epoch": 51, "loss": 1.9, "agreement": 0.75}])
        assert rosame_series(inst)["ROSAME_MILP_24"]["agreement"] == [None, 0.75]
