...
(:action move_left
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-up ?from ?to) (move-dir-up ?to ?from) (move-dir-left ?to ?from) (move-dir-right ?from ?to) (move-dir-right ?to ?from) (clear ?from) (clear ?to) (at ?p ?from) (at ?p ?to) (oriented-up ?p) (oriented-down ?p) (oriented-left ?p) (oriented-right ?p) (is-goal ?from) (is-goal ?to))
	:effect (and (move-dir-down ?from ?to) (move-dir-down ?to ?from) (move-dir-left ?from ?to) (not (move-dir-up ?from ?to))  (not (move-dir-up ?to ?from))  (not (move-dir-left ?to ?from))  (not (move-dir-right ?from ?to))  (not (move-dir-right ?to ?from))  (not (clear ?from))  (not (clear ?to))  (not (at ?p ?from))  (not (at ?p ?to))  (not (oriented-up ?p))  (not (oriented-down ?p))  (not (oriented-left ?p))  (not (oriented-right ?p))  (not (is-goal ?from))  (not (is-goal ?to))))
