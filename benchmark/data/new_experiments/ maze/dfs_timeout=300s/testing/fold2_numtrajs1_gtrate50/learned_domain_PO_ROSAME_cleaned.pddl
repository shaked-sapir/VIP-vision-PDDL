...
(:action move_left
	...
	:effect (and (move-dir-left ?from ?to) (move-dir-right ?to ?from) (clear ?from) (at ?p ?to) (oriented-left ?p) (not (clear ?to))  (not (at ?p ?from))))

(:action move_right
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and )
	:effect (and (move-dir-down ?from ?to) (move-dir-left ?to ?from) (move-dir-right ?from ?to) (move-dir-right ?to ?from) (clear ?from) (at ?p ?from) (oriented-up ?p)))
