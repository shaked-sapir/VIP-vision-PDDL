...
(:action move_left
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-right ?to ?from) (clear ?to) (at ?p ?from) (oriented-right ?p))
	:effect (and (move-dir-left ?from ?to) (clear ?from) (at ?p ?to) (oriented-left ?p) (not (clear ?to))  (not (at ?p ?from))  (not (oriented-right ?p))))
