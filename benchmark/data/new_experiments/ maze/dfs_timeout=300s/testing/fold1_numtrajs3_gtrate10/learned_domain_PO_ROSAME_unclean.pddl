...
(:action move_up
	...
	:precondition (and (move-dir-down ?from ?to) (move-dir-right ?to ?from) (clear ?to) (oriented-right ?p) (is-goal ?to))
	:effect (and ))

(:action move_down
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-up ?to ?from) (clear ?to) (at ?p ?from))
	:effect (and (move-dir-down ?from ?to) (clear ?from) (at ?p ?to) (oriented-down ?p) (not (clear ?to))  (not (at ?p ?from))))
