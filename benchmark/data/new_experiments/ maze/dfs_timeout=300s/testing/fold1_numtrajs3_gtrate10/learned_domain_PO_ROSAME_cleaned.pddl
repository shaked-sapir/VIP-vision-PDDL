...
(:action move_left
	...
	:effect (and (move-dir-right ?to ?from) (clear ?from) (at ?p ?to) (oriented-left ?p) (not (clear ?to))  (not (at ?p ?from))  (not (oriented-right ?p))))

(:action move_right
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (clear ?to) (at ?p ?from) (oriented-down ?p))
	:effect (and (move-dir-right ?from ?to) (clear ?from) (at ?p ?to) (oriented-right ?p) (not (clear ?to))  (not (at ?p ?from))  (not (oriented-down ?p))))
