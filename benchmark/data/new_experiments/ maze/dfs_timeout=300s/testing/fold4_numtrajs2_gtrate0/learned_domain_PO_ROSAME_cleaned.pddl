...
(:action move_down
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (clear ?to) (at ?p ?from) (oriented-up ?p) (oriented-right ?p))
	:effect (and (clear ?from) (at ?p ?to) (not (clear ?to))  (not (at ?p ?from))  (not (oriented-up ?p))  (not (oriented-right ?p))))
