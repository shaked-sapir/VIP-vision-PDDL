...
(:action move_right
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-right ?from ?to) (clear ?to) (at ?p ?from) (oriented-down ?p))
	:effect (and (move-dir-left ?to ?from) (clear ?from) (at ?p ?to) (oriented-right ?p) (not (clear ?to))  (not (at ?p ?from))  (not (oriented-down ?p))))
