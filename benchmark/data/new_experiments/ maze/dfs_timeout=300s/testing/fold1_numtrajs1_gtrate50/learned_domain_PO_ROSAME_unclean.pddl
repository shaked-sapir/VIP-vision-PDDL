...
(:action move_left
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-down ?to ?from) (move-dir-right ?from ?to) (clear ?from) (at ?p ?from) (at ?p ?to) (oriented-up ?p) (is-goal ?from))
	:effect (and  (not (move-dir-down ?to ?from))  (not (move-dir-right ?from ?to))  (not (clear ?from))  (not (at ?p ?from))  (not (at ?p ?to))  (not (oriented-up ?p))  (not (is-goal ?from))))
