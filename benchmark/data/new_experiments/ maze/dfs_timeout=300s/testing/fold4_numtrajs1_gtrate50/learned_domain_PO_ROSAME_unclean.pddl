...
(:action move_right
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-down ?to ?from) (move-dir-left ?to ?from) (clear ?from) (at ?p ?from) (oriented-down ?p))
	:effect (and (move-dir-up ?from ?to) (move-dir-up ?to ?from) (move-dir-down ?from ?to) (move-dir-left ?from ?to) (move-dir-right ?from ?to) (move-dir-right ?to ?from) (clear ?to) (at ?p ?to) (oriented-up ?p) (oriented-left ?p) (oriented-right ?p) (is-goal ?from) (is-goal ?to) (not (move-dir-down ?to ?from))  (not (move-dir-left ?to ?from))  (not (clear ?from))  (not (at ?p ?from))  (not (oriented-down ?p))))
