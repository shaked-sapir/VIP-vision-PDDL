...
(:action move_down
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-up ?to ?from) (move-dir-down ?from ?to) (clear ?to) (at ?p ?from) (oriented-down ?p))
...
(:action move_right
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-up ?to ?from) (move-dir-down ?from ?to) (move-dir-down ?to ?from) (move-dir-left ?from ?to) (move-dir-left ?to ?from) (move-dir-right ?from ?to) (move-dir-right ?to ?from) (clear ?from) (clear ?to) (at ?p ?from) (oriented-up ?p) (oriented-down ?p) (oriented-left ?p) (oriented-right ?p) (is-goal ?from) (is-goal ?to))
	:effect (and  (not (move-dir-up ?to ?from))  (not (move-dir-down ?to ?from))  (not (move-dir-left ?to ?from))  (not (move-dir-right ?from ?to))  (not (move-dir-right ?to ?from))  (not (clear ?from))  (not (clear ?to))  (not (at ?p ?from))  (not (oriented-up ?p))  (not (oriented-down ?p))  (not (oriented-left ?p))  (not (oriented-right ?p))  (not (is-goal ?from))  (not (is-goal ?to))))
