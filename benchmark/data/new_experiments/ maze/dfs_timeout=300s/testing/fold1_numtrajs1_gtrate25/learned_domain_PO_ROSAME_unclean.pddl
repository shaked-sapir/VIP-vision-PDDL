...
(:action move_left
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-down ?from ?to) (is-goal ?from) (is-goal ?to))
	:effect (and (move-dir-up ?from ?to) (move-dir-up ?to ?from) (move-dir-down ?to ?from) (move-dir-left ?from ?to) (move-dir-left ?to ?from) (move-dir-right ?from ?to) (move-dir-right ?to ?from) (clear ?from) (clear ?to) (at ?p ?from) (at ?p ?to) (oriented-up ?p) (oriented-down ?p) (oriented-left ?p) (oriented-right ?p) (not (move-dir-down ?from ?to))))
