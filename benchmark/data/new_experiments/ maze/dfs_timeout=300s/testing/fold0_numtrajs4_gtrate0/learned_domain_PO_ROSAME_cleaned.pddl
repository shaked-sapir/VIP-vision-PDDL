...
(:action move_down
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-down ?from ?to) (move-dir-right ?from ?to) (clear ?from) (is-goal ?to))
	:effect (and  (not (move-dir-down ?from ?to))  (not (move-dir-right ?from ?to))  (not (clear ?from))  (not (is-goal ?to))))

(:action move_left
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-left ?from ?to) (move-dir-right ?to ?from) (clear ?from) (oriented-up ?p))
