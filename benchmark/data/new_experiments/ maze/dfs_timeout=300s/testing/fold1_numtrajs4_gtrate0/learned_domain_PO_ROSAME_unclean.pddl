...
(:action move_up
	...
	:effect (and (move-dir-down ?to ?from) (clear ?from) (at ?p ?to) (oriented-up ?p) (not (clear ?to))  (not (at ?p ?from))  (not (oriented-left ?p))))

(:action move_down
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and )
	:effect (and (move-dir-up ?from ?to) (move-dir-up ?to ?from) (move-dir-left ?from ?to) (move-dir-left ?to ?from) (move-dir-right ?from ?to) (move-dir-right ?to ?from) (clear ?from) (clear ?to) (at ?p ?from) (at ?p ?to) (oriented-up ?p) (oriented-down ?p) (oriented-left ?p) (is-goal ?from)))
