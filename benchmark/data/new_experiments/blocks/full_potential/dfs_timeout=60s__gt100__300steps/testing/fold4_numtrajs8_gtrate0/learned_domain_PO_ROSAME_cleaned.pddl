...
(:action stack
	...
	:precondition (and (ontable ?y) (clear ?x) (holding ?x))
	:effect (and (on ?y ?x) (ontable ?x) (handempty) (not (ontable ?y))  (not (clear ?x))  (not (holding ?x))))

(:action unstack
	:parameters (?x - block ?y - block)
	:precondition (and (on ?x ?y) (on ?y ?x) (ontable ?x) (ontable ?y) (clear ?x) (clear ?y) (handempty) (holding ?x) (holding ?y))
	:effect (and  (not (on ?x ?y))  (not (on ?y ?x))  (not (ontable ?x))  (not (ontable ?y))  (not (clear ?x))  (not (handempty))  (not (holding ?x))))
