...
(:action unstack
	:parameters (?x - block ?y - block)
	:precondition (and (on ?x ?y) (ontable ?x) (clear ?x) (handempty) (holding ?y))
	:effect (and (on ?y ?x) (ontable ?y) (clear ?y) (holding ?x) (not (on ?x ?y))  (not (ontable ?x))  (not (clear ?x))  (not (handempty))  (not (holding ?y))))
