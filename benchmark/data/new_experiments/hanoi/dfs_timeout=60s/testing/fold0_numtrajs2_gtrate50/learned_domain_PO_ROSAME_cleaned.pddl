...
(:action move_disc_peg
	...
	:precondition (and (clear-disc ?from) (on-disc ?from ?disc) (on-peg ?disc ?to) (on-peg ?from ?to) (smaller-peg ?to ?disc))
	:effect (and (clear-peg ?to) (clear-disc ?disc) (on-disc ?disc ?from) (smaller-disc ?disc ?from) (smaller-disc ?from ?disc) (smaller-peg ?to ?from) (not (clear-disc ?from))  (not (on-disc ?from ?disc))  (not (on-peg ?disc ?to))  (not (on-peg ?from ?to))  (not (smaller-peg ?to ?disc))))
