...
(:action move-down
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and )
	:effect (and (move-dir-right ?to ?from) (clear ?to) (oriented-left ?p) (oriented-right ?p)))

(:action move-left
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (at ?p ?to) (oriented-up ?p))
	:effect (and (move-dir-left ?from ?to) (move-dir-right ?to ?from) (clear ?from) (oriented-right ?p) (not (oriented-up ?p))))
