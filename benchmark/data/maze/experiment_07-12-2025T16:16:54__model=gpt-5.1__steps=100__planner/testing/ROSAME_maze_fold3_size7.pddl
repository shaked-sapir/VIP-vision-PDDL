...
(:action move_left
	...
	:effect (and (at ?p ?to) (oriented-left ?p) (not (clear ?to))  (not (oriented-up ?p))))

(:action move_right
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-left ?from ?to) (move-dir-left ?to ?from) (move-dir-right ?from ?to) (move-dir-right ?to ?from) (clear ?to) (at ?p ?from) (oriented-up ?p) (oriented-down ?p))
	:effect (and (move-dir-up ?from ?to) (move-dir-up ?to ?from) (move-dir-down ?from ?to) (move-dir-down ?to ?from) (clear ?from) (at ?p ?to) (oriented-left ?p) (oriented-right ?p) (is-goal ?from) (is-goal ?to) (not (move-dir-left ?from ?to))  (not (move-dir-right ?to ?from))  (not (at ?p ?from))  (not (oriented-up ?p))  (not (oriented-down ?p))))
