...
(:action move-up
	...
	:precondition (and )
	:effect (and (move-dir-right ?from ?to) (clear ?to) (at ?p ?to) (oriented-right ?p) (is-goal ?to)))

(:action move-down
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-up ?to ?from) (move-dir-down ?from ?to) (clear ?to) (at ?p ?from) (oriented-left ?p))
	:effect (and (move-dir-up ?from ?to) (move-dir-down ?to ?from) (move-dir-left ?from ?to) (move-dir-left ?to ?from) (move-dir-right ?from ?to) (move-dir-right ?to ?from) (clear ?from) (at ?p ?to) (oriented-up ?p) (oriented-down ?p) (oriented-right ?p) (is-goal ?from) (is-goal ?to) (not (oriented-left ?p))))
