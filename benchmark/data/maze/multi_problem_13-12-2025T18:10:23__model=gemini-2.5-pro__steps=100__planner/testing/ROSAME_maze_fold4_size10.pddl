...
(:action move-up
	...
	:effect (and (clear ?from) (at ?p ?to) (oriented-up ?p) (oriented-left ?p) (not (clear ?to))  (not (at ?p ?from))  (not (oriented-right ?p))))
...
(:action move-left
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-left ?from ?to) (move-dir-right ?to ?from) (clear ?to) (at ?p ?from) (oriented-up ?p) (is-goal ?to))
	:effect (and (move-dir-up ?from ?to) (move-dir-up ?to ?from) (move-dir-down ?from ?to) (move-dir-down ?to ?from) (move-dir-left ?to ?from) (clear ?from) (at ?p ?to) (oriented-down ?p) (oriented-right ?p) (not (clear ?to))  (not (at ?p ?from))  (not (oriented-up ?p))  (not (is-goal ?to))))
