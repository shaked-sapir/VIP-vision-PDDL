...
(:action move-up
	...
	:effect (and (at ?p ?from) (at ?p ?to) (oriented-up ?p) (oriented-right ?p) (not (oriented-left ?p))))

(:action move-down
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (oriented-left ?p) (is-goal ?from))
	:effect (and (move-dir-up ?from ?to) (move-dir-up ?to ?from) (move-dir-down ?from ?to) (move-dir-down ?to ?from) (move-dir-left ?from ?to) (move-dir-left ?to ?from) (move-dir-right ?from ?to) (move-dir-right ?to ?from) (clear ?from) (clear ?to) (at ?p ?from) (at ?p ?to) (oriented-up ?p) (oriented-down ?p) (oriented-right ?p) (is-goal ?to)))

(:action move-left
	:parameters (?p - player ?from - location ?to - location)
	:precondition (and (move-dir-left ?from ?to) (move-dir-right ?to ?from) (clear ?from) (clear ?to) (oriented-right ?p))
	:effect (and (at ?p ?to)))
