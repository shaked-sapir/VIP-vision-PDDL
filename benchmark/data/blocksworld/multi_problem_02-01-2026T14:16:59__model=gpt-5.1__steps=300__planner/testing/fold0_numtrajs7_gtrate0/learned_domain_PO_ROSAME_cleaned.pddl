...
(:action stack
	...
	:effect (and (on ?x ?y) (on ?y ?x) (handempty) (not (clear ?x))  (not (clear ?y))  (not (holding ?x))))

(:action unstack
	:parameters (?x - block ?y - block)
	:precondition (and (on ?x ?y) (ontable ?y) (handempty))
	:effect (and (clear ?y) (holding ?x) (not (on ?x ?y))  (not (ontable ?y))  (not (handempty))))
