...
(:action unstack
	:parameters (?x - block ?y - block)
	:precondition (and (on ?x ?y) (on ?y ?x) (handempty) (holding ?x))
	:effect (and (ontable ?x) (ontable ?y) (clear ?x) (clear ?y) (holding ?y) (not (on ?x ?y))  (not (on ?y ?x))  (not (handempty))  (not (holding ?x))))
