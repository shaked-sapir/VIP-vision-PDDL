...
in `benchmark/algorithm_adapters/rosame_milp/__init__.py` so upstream's
...
(`benchmark/algorithm_adapters/rosame_milp/encoder.py`) and is registered in
the same factory under `"cp-sat-observed"`.
