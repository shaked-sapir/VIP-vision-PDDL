...
from src.utils.pddl import propagate_frame_axioms_in_trajectory, observation_to_trajectory_file
    ...
    for dir_name in experiment_data_dirs[domain_name]:
        ...
        for fold in range(N_FOLDS):
            ...
            for num_trajs_used in traj_settings:
                ...
                noisy_pisam_model, final_observations = noisy_pisam.learn(domain_ref_path, selected_trajs)
                for obs, traj_path in zip(final_observations, selected_trajs):
                    observation_to_trajectory_file(obs, Path(traj_path).with_suffix('.final.trajectory'))

                # see that we can reconstruct the trajectories with frame axioms propagated
                print("now learning with the new final trajectory only (to verify frame axioms propagation)...")
                noisy_pisam_model, final_observations = noisy_pisam.learn(domain_ref_path, ["/Users/shakedsapir/Documents/BGU/thesis/VIP-vision-PDDL/benchmark/data/blocksworld/multi_problem_04-12-2025T12:00:44__model=gpt-5.1__steps=50__planner/training/trajectories/problem7/problem7.final.trajectory"])
