...
The feature spans two layers on purpose: ``expand_cdps_milp_ablations`` lives in
...
def _labels(key: str, raw: dict) -> list:
    ...
    return [
        cdps_milp_algorithm_name(config)
        for config in milp_configs_for(key, expand_cdps_milp_ablations(raw))
    ]


def test_no_ablations_is_one_unchanged_config() -> None:
    """A block without ``ablations:`` behaves exactly as it did before."""
    configs = expand_cdps_milp_ablations(_BASE)
    assert len(configs) == 1, configs
    assert configs[0] == CdpsMilpConfig.from_dict(_BASE)

    # And an absent block is still the default config, not an empty list — the
    # MILP arms have to be runnable with no `cdps_milp:` at all.
    assert expand_cdps_milp_ablations(None) == [CdpsMilpConfig()]
    assert expand_cdps_milp_ablations({}) == [CdpsMilpConfig()]
...
def test_ablation_is_a_cross_product() -> None:
    ...
    configs = expand_cdps_milp_ablations(raw)
...
def test_ablation_overrides_rather_than_extends() -> None:
    ...
    """
    configs = expand_cdps_milp_ablations(dict(_BASE, ablations={"eq16": [True]}))
    assert len(configs) == 1, configs
...
def test_loop_only_knobs_collapse_under_single_round() -> None:
    ...
    assert _labels(CDPS_MILP_LOOP, raw) == [
        "CDPS_MILP_LOOP", "CDPS_MILP_LOOP__pool=replace",
    ]
    assert _labels(CDPS_MILP_SINGLE_ROUND, raw) == ["CDPS_MILP_SR"]
...
def test_shared_knob_splits_both_arms() -> None:
    ...
    assert _labels(CDPS_MILP_LOOP, raw) == [
        "CDPS_MILP_LOOP", "CDPS_MILP_LOOP__eq16=0.4",
    ]
    assert _labels(CDPS_MILP_SINGLE_ROUND, raw) == [
        "CDPS_MILP_SR", "CDPS_MILP_SR__eq16=0.4",
    ]
...
def test_variant_comes_from_the_key_not_the_config() -> None:
    ...
    configs = expand_cdps_milp_ablations(_BASE)
    assert milp_configs_for(CDPS_MILP_LOOP, configs)[0].variant is MilpVariant.LOOP
    assert (milp_configs_for(CDPS_MILP_SINGLE_ROUND, configs)[0].variant
...
def test_label_collision_is_rejected() -> None:
    ...
    the two arms would be averaged into one ``CDPS_MILP_LOOP`` row.
    """
    configs = expand_cdps_milp_ablations(dict(_BASE, ablations={"seed": [1, 2]}))
    assert len(configs) == 2, "the expansion itself is fine; selection is not"
    try:
        milp_configs_for(CDPS_MILP_LOOP, configs)
    except ValueError as err:
        assert "CDPS_MILP_LOOP" in str(err), err
    else:
...
def test_unablatable_and_malformed_blocks_are_rejected() -> None:
    ...
    for name, ablations in cases.items():
        try:
            expand_cdps_milp_ablations(dict(_BASE, ablations=ablations))
...
def test_work_subdir_matches_the_label_and_keeps_the_old_default() -> None:
    ...
    """
    default = expand_cdps_milp_ablations(_BASE)[0]
    assert milp_work_subdir(CDPS_MILP_LOOP, default) == "cdps_milp_loop"
    assert (milp_work_subdir(CDPS_MILP_SINGLE_ROUND, default)
            == "cdps_milp_single_round")

    ablated = expand_cdps_milp_ablations(
        dict(_BASE, ablations={"pool_policy": ["replace"]}))[0]
    assert (milp_work_subdir(CDPS_MILP_LOOP, ablated)
            == "cdps_milp_loop__pool=replace")

    # Distinct arms must not share a directory, or the second overwrites the
    # first's artifacts inside one fold.
    raw = dict(_BASE, ablations={"pool_policy": ["frozen", "replace"]})
    dirs = [milp_work_subdir(CDPS_MILP_LOOP, c)
            for c in milp_configs_for(CDPS_MILP_LOOP, expand_cdps_milp_ablations(raw))]
